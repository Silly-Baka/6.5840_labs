...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("leader [%v] dead 1", leader1)
	...
	DPrintf("dead leader [%v] reconnect 1", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no new leader should
	// be elected.
	DPrintf("leader [%v] dead 2", leader2)
	cfg.disconnect(leader2)

	DPrintf("leader [%v] dead 3", (leader2+1)%servers)
	...
	DPrintf("dead leader [%v] reconnect 2", (leader2+1)%servers)
	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	DPrintf("dead leader [%v] reconnect 3", leader2)
...
func TestLeaderFailure2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf("leader %v was disconnected", leader1)
	cfg.disconnect(leader1)
	...
	leader2 := cfg.checkOneLeader()
	DPrintf("leader %v was disconnected", leader2)
	cfg.disconnect(leader2)
...
func TestFailAgree2B(t *testing.T) {
	...
	DPrintf("[%v] follower was disconnected", (leader+1)%servers)
	...
	// re-connect
	DPrintf("[%v] follower was reconnected", (leader+1)%servers)
...
func TestFailNoAgree2B(t *testing.T) {
	...
	leader := cfg.checkOneLeader()
	DPrintf("follower [%v] disconnected", (leader+1)%servers)
	cfg.disconnect((leader + 1) % servers)
	DPrintf("follower [%v] disconnected", (leader+2)%servers)
	cfg.disconnect((leader + 2) % servers)
	DPrintf("follower [%v] disconnected", (leader+3)%servers)
	...
	// repair
	DPrintf("follower [%v] reconnected", (leader+1)%servers)
	cfg.connect((leader + 1) % servers)
	DPrintf("follower [%v] reconnected", (leader+2)%servers)
	cfg.connect((leader + 2) % servers)
	DPrintf("follower [%v] reconnected", (leader+3)%servers)
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf("old leader %v disconnect", leader1)
	cfg.disconnect(leader1)
	...
	leader2 := cfg.checkOneLeader()
	DPrintf("new leader %v disconnect", leader2)
	cfg.disconnect(leader2)
	...
	// all together now
	DPrintf("new leader %v reconnect", leader2)
	cfg.connect(leader2)
...
func TestBackup2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf("follower %v was disconnected", (leader1+2)%servers)
	DPrintf("follower %v was disconnected", (leader1+3)%servers)
	DPrintf("follower %v was disconnected", (leader1+4)%servers)
	...
	DPrintf("%v was disconnected", (leader1+0)%servers)
	DPrintf("%v was disconnected", (leader1+1)%servers)
	cfg.disconnect((leader1 + 0) % servers)
	cfg.disconnect((leader1 + 1) % servers)

	// allow other partition to recover
	DPrintf("%v was connected", (leader1+2)%servers)
	DPrintf("%v was connected", (leader1+3)%servers)
	DPrintf("%v was connected", (leader1+4)%servers)
	...
	DPrintf("%v was disconnected", other)
	...
	for i := 0; i < servers; i++ {
		DPrintf("%v was disconnected", i)
		cfg.disconnect(i)
	}
	DPrintf("%v was connected", (leader1+0)%servers)
	DPrintf("%v was connected", (leader1+1)%servers)
	DPrintf("%v was connected", other)
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		if leader != -1 && (rand.Int()%1000) < int(RaftElectionTimeout/time.Millisecond)/2 {
			DPrintf("leader %v was diconnected", leader)
		...
		if nup < 3 {
			...
			if cfg.connected[s] == false {
				DPrintf("peer %v was connected", s)
				cfg.connect(s)
				nup += 1
			}
		}
	...
	for i := 0; i < servers; i++ {
		if cfg.connected[i] == false {
			DPrintf("peer %v was connected", i)
			cfg.connect(i)
		}
	}
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			DPrintf("%v was disconnected", victim)
			cfg.disconnect(victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		if crash {
			DPrintf("%v was crash", victim)
			cfg.crash1(victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		...
		// let applyBufferCh threads catch up with the Start()'s
		...
		if disconnect {
			...
			DPrintf("%v was connected", victim)
			cfg.connect(victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
			DPrintf("%v was restart", victim)
